...
// â”€â”€â”€ Ã‰tat global â”€â”€â”€
let zIndex = 100;
let dragState = null;   // { type, element, offsetX, offsetY }
let windowCount = 0;
const DESKTOP = document.getElementById('desktop');
const openWindows = {}; // type â†’ element, pour Ã©viter les doublons

const isMobile = window.matchMedia('(max-width: 768px), (max-height: 600px)').matches;

// Sur mobile : verrouiller le scroll dÃ¨s le dÃ©part
if (isMobile) {
...
// â”€â”€â”€ Clic â†’ fenÃªtre (desktop) â”€â”€â”€
if (!isMobile) {
icons.forEach(icon => {
...
// â”€â”€â”€ Mobile : appui long â†’ drag, tap simple â†’ fenÃªtre, swipe â†’ rotation cercle â”€â”€â”€
if (isMobile) {
...
icons.forEach(icon => {
  ...
  icon.addEventListener('touchmove', e => {
    if (longPressActive) {
      ...
      if (iconSwipeActive) {
        ...
        swipePrevX = t.clientX;
        swipeLastDx = 0;
        // Pas de circlePaused ici : le cercle continue de tourner lÃ©gÃ¨rement pendant le swipe
        const totalDx = t.clientX - swipeRefX;
        circleAngle = swipeStartAngle - totalDx * 0.004;
        updateCirclePositions();
        e.preventDefault();
  ...
  icon.addEventListener('touchend', e => {
    ...
    if (longPressActive) {
      ...
      // Fin du swipe : appliquer le momentum basÃ© sur le dernier delta
      scrollVel = -swipeLastDx * 0.6;
      iconSwipeActive = false;
    } else if (touchStartPos) {
      // Tap simple â†’ ouvre la fenÃªtre
      const type = icon.dataset.window;
      if (type) openWindow(type);
    }
    touchStartPos = null;
...
// â”€â”€â”€ FenÃªtres â”€â”€â”€
function openWindow(type) {
  // Ã‰vite les doublons : si une fenÃªtre du mÃªme type existe dÃ©jÃ , on la ramÃ¨ne au premier plan
  if (openWindows[type] && document.body.contains(openWindows[type])) {
    ...
    // marquer la fenÃªtre comme focus
  ...
  const sizes = {
    ...
    kanaga: { w: 380, h: 675, title: 'KANAGA<wbr>Foule DÃ©sirs' },
    ...
    plus33logos:{ w: 720, h: 454, title: 'PLUS33<wbr>Logos AnimÃ©es' },
      ...
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block"><span class="italic">&laquo;&nbsp;Mono no Aware&nbsp;&raquo;</span> est un concept esthÃ©tique et spirituel japonais, pouvant Ãªtre traduit comme &laquo;&nbsp;l'empathie envers les choses&nbsp;&raquo; ou &laquo;&nbsp;la sensibilitÃ© pour l'Ã©phÃ©mÃ¨re&nbsp;&raquo;. <span class="dots">...</span></span>

            <div class="hidden text-gray-500 mt-1 leading-relaxed space-y-1">
              <span class="block">RÃ©alisation : Julien ROOSE, Anthony VUILLEROT</span>
              <span class="block">ScÃ©nario : Julien ROOSE</span>
              ...
              <span class="block">Ã‰talonnage : Mathieu BOUVY</span>
              <span class="block">Mixage son : Anthony VUILLEROT</span>
              <span class="block">Remerciements Ã  : Thomas AUBERTIN, ClÃ©ment DAZIN, OcÃ©ane DUMAS, Marie NIETO, Alexandre ROOSE, Didier ROOSE</span>
            ...
            <button onclick="var d=this.previousElementSibling,dots=d.previousElementSibling.querySelector('.dots');d.classList.toggle('hidden');dots.classList.toggle('hidden');this.textContent=d.classList.contains('hidden')?'â–¼ DÃ©velopper les crÃ©dits':'â–² RÃ©duire les crÃ©dits'"
                    class="text-gray-600 hover:text-gray-400 transition-colors mt-1 block text-xs">
              â–¼ DÃ©velopper les crÃ©dits
            </button>
          </div>
        </div>`;
      break;
    ...
    case 'puma':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Retour en image sur la Draft organisÃ©e par @said_piedscarres et @yannoujr pour sÃ©lectionner les meilleures pÃ©pites pour la 1vs1 Cup cet Ã©tÃ©. <span class="dots">...</span></span>
    ...
    case 'ol':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Les coulisses de notre shooting Third avec les commerÃ§ants et habitants du Vieux Lyon ðŸŽ¥ <span class="dots">...</span></span>
    ...
    case 'youtube':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Raska et Isia s'affrontent pour trouver la bonne boÃ®te et repartir avec un mystÃ©rieux cadeau. <span class="dots">...</span></span>

            <div class="hidden text-gray-500 mt-1 leading-relaxed space-y-1">
              <span class="block text-gray-400">Qui sera le plus stratÃ¨ge et repartira avec la bonne boÃ®te ? La rÃ©ponse est dans cet Ã©pisodeâ€¦</span>
              <span class="block">PrÃ©sentateur : @ryan_mahie</span>
              <span class="block">RÃ©alisation : @anthony.debrant @justiine.v @ryan_mahie</span>
    ...
    case 'eafut':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Quel ballon d'or est nÃ© le 17.04.01 ? <span class="dots">...</span></span>
      ...
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            ...
            <button onclick="var d=this.previousElementSibling,dots=d.previousElementSibling.querySelector('.dots');d.classList.toggle('hidden');dots.classList.toggle('hidden');this.textContent=d.classList.contains('hidden')?'â–¼ Developper les credits':'â–² Reduire les credits'"
                    class="text-gray-600 hover:text-gray-400 transition-colors mt-1 block text-xs">
              â–¼ Developper les credits
            </button>
          </div>
        </div>`;
      break;
        ...
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block italic leading-relaxed">
              Comme un semblant d'harmonie dans le dÃ©sordre,<br>
              Ã  travers la frÃ©nÃ©sie du bÃ©ton <span class="dots">...</span>
            </span>
    ...
    case 'cv':
      bodyHTML = `
        <div class="flex h-full text-sm">
          ...
          <!-- â”€â”€â”€ Colonne gauche : contact / compÃ©tences â”€â”€â”€ -->
          <div class="w-[35%] bg-white/[0.03] p-5 flex flex-col gap-5 flex-shrink-0 overflow-y-auto">
            ...
            <div>
              ...
              <div class="space-y-1.5 text-xs text-gray-400">
                ...
                <p class="text-gray-500 text-xs">VidÃ©aste</p>
            ...
            <!-- CompÃ©tences -->
            <div>
              <h3 class="text-xs font-semibold uppercase tracking-wider mb-2" style="color:#38bdf8">CompÃ©tences</h3>
              <div class="flex flex-wrap gap-1.5">
                <span class="px-2.5 py-1 text-xs rounded-md bg-white/5 text-gray-300" style="border:1px solid rgba(56,189,248,0.15);">Montage vidÃ©o</span>
                ...
                <span class="px-2.5 py-1 text-xs rounded-md bg-white/5 text-gray-300" style="border:1px solid rgba(56,189,248,0.15);">Jeu VidÃ©o</span>
            ...
            <div>
              ...
              <div class="space-y-1 text-xs text-gray-400">
                <p>FranÃ§ais <span class="text-gray-600">â€” Natif</span></p>
                <p>Anglais <span class="text-gray-600">â€” Professionnel</span></p>
              </div>
            </div>
          </div>

          <!-- â”€â”€â”€ Colonne droite : expÃ©riences / formations â”€â”€â”€ -->
          <div class="flex-1 p-5 flex flex-col gap-5 overflow-y-auto">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">VidÃ©aste</p>
            </div>

            <!-- ExpÃ©riences -->
            <div>
              <h3 class="text-xs font-semibold uppercase tracking-wider mb-3" style="color:#a78bfa">ExpÃ©riences</h3>
              <div class="space-y-4">
                ...
                <div class="border-l-2 pl-3" style="border-color:rgba(167,139,250,0.3)">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <span class="text-gray-600 text-xs whitespace-nowrap pt-0.5">DÃ©c 2022 â€” Sept 2024</span>
                  </div>
                ...
                <div class="border-l-2 pl-3" style="border-color:rgba(167,139,250,0.3)">
                  <div class="flex items-start justify-between gap-2">
                    ...
                    <span class="text-gray-600 text-xs whitespace-nowrap pt-0.5">Sept 2023 â€” Oct 2025</span>
                  </div>
                  <p class="text-gray-500 text-xs mt-0.5">VidÃ©aste / Motion Designer <span class="text-gray-600">(Alternance)</span></p>
                  <p class="text-gray-500 text-xs mt-1 leading-relaxed">Projets sport, lifestyle et jeux vidÃ©o.</p>
                </div>

                <div class="border-l-2 pl-3" style="border-color:rgba(167,139,250,0.3)">
                  <div class="flex items-start justify-between gap-2">
                    <p class="text-white font-medium text-sm">Freelance</p>
                    <span class="text-gray-600 text-xs whitespace-nowrap pt-0.5">Oct 2025 â€” PrÃ©sent</span>
                  </div>
                  <p class="text-gray-500 text-xs mt-0.5">VidÃ©aste / Monteur indÃ©pendant</p>
                  <p class="text-gray-500 text-xs mt-1 leading-relaxed">SpÃ©cialisÃ© dans les projets sport, lifestyle et jeux vidÃ©o. Collaboration avec des agences et marques en production audiovisuelle.</p>
                </div>
            ...
            <div>
              ...
              <div class="space-y-3">
                <div class="border-l-2 pl-3" style="border-color:rgba(167,139,250,0.3)">
                  ...
                  <p class="text-gray-500 text-xs">2021 â€” 2024 Â· Ynov Lyon</p>
                </div>
                <div class="border-l-2 pl-3" style="border-color:rgba(167,139,250,0.3)">
                  <p class="text-white text-sm">BaccalaurÃ©at GÃ©nÃ©ral</p>
                  <p class="text-gray-500 text-xs">Option CinÃ©ma Audiovisuel</p>
                </div>
              </div>
            </div>
    ...
    case 'adidas':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Adidas â€” Vintage Market.</span>
          </div>
        </div>`;
      break;
    ...
    case 'highlo':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">HighLo â€” Bushi.</span>
          </div>
        </div>`;
      break;
    ...
    case 'jumpman':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Jumpman â€” fx.</span>
          </div>
        </div>`;
      break;
        ...
        <div class="flex flex-col h-full">
          <div class="flex-1 min-h-0 bg-black/40 rounded-lg overflow-hidden mb-3">
            <iframe class="w-full h-full" src="https://www.youtube.com/embed/QhXHbWDuPns?autoplay=1"
                    title="KANAGA_Foule DÃ©sirs" frameborder="0"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                    referrerpolicy="origin" allowfullscreen>
            </iframe>
          </div>
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Kanaga â€” Foule DÃ©sirs.</span>
          </div>
        </div>`;
      break;
    ...
    case 'laligue':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">LaLigue â€” Lyon street food festival.</span>
          </div>
        </div>`;
      break;
    ...
    case 'pola':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">LaLigue â€” Pola freestyle.</span>
          </div>
        </div>`;
      break;
    ...
    case 'nikon2024':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">NIKON â€” 2024.</span>
          </div>
        </div>`;
      break;
    ...
    case 'pumacrampons':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">PUMA â€” Crampons.</span>
          </div>
        </div>`;
      break;
    ...
    case 'wallace':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Punchologue â€” Comprendre Baiser de Wallace Cleaver.</span>
          </div>
        </div>`;
      break;
    ...
    case 'sofianee':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Sofianee â€” Gold digger.</span>
          </div>
        </div>`;
      break;
    ...
    case 'udol':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">UDOL â€” Gameday.</span>
          </div>
        </div>`;
      break;
    ...
    case 'unibet':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Unibet â€” Greg MMA.</span>
          </div>
        </div>`;
      break;
    ...
    case 'unibetnasri':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Unibet â€” Greg MMA x SAmir NASRI.</span>
          </div>
        </div>`;
      break;
    ...
    case 'anooki':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Ville de Lyon â€” Anooki.</span>
          </div>
        </div>`;
      break;
    ...
    case 'whentocop':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Whentocop â€” Nike Air max.</span>
          </div>
        </div>`;
      break;
    ...
    case 'plus33fm':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Plus33FM â€” Motion.</span>
          </div>
        </div>`;
      break;
    ...
    case 'olmaillot':
      bodyHTML = `
        <div class="flex flex-col h-full">
          ...
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">OL â€” Nouveau maillot extÃ©rieur.</span>
          </div>
        </div>`;
      break;
        ...
        <div class="flex flex-col h-full">
          <div class="flex-1 min-h-0 bg-black/40 rounded-lg overflow-hidden mb-3">
            <iframe class="w-full h-full" src="https://www.youtube.com/embed/OLX8yLoVE88?autoplay=1"
                    title="PLUS33_Logos AnimÃ©es" frameborder="0"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                    referrerpolicy="origin" allowfullscreen>
            </iframe>
          </div>
          <div class="flex-shrink-0 text-xs text-gray-400 leading-relaxed space-y-1 px-5 pb-4">
            <span class="block">Plus33FM â€” Logos AnimÃ©es.</span>
          </div>
        </div>`;
      break;
  ...
  win.innerHTML = `
    <div class="window-titlebar">
      <div class="window-controls">
        ...
        <button class="window-btn window-btn-minimize" data-action="minimize" title="RÃ©duire"></button>
  ...
  // â”€â”€â”€ Boutons titre (fermer, rÃ©duire, agrandir) â”€â”€â”€
  ...
  // â”€â”€â”€ Drag de fenÃªtre (par la titlebar) â”€â”€â”€
  if (!isMobile) {
...
// â”€â”€â”€ Restaurer la taille initiale (bouton jaune) â”€â”€â”€
...
// â”€â”€â”€ Agrandir / restaurer â”€â”€â”€
...
// â”€â”€â”€ Mouse move / up globaux â”€â”€â”€
document.addEventListener('mousemove', e => {
  ...
  // â”€â”€â”€ Drag icÃ´ne / fenÃªtre â”€â”€â”€
  const doff = DESKTOP.getBoundingClientRect();
  let x = e.clientX - dragState.offsetX - doff.left;
  let y = e.clientY - dragState.offsetY - doff.top;

  // DÃ©tacher du cercle si le drag dÃ©passe 10px
  if (dragState.type === 'icon') {
    const dx = e.clientX - dragState.startX;
    const dy = e.clientY - dragState.startY;
    // Marquer comme dragÃ© pour Ã©viter l'ouverture de fenÃªtre
    ...
    if (!dragState.detached && dragState.element.dataset.category && dx*dx + dy*dy > 100) {
      ...
      // Si le cercle Ã©tait en pause, le relancer
      circlePaused = false;
    }
  }
...
document.addEventListener('touchend', () => {
  // SÃ©curitÃ© : cleanup dragState au cas oÃ¹ touchend sur l'icÃ´ne n'a pas marchÃ©
...
// â”€â”€â”€ Filtre par catÃ©gorie (RÃ©alisation / Post-Production) â”€â”€â”€
...
function filterCategory(cat) {
  ...
  // DÃ©sactivation si on reclique sur la mÃªme catÃ©gorie
  ...
  // Appliquer le filtre (CV et Contact sans catÃ©gorie restent toujours visibles)
  icons.forEach(icon => {
...
// â”€â”€â”€ Variables globales pour la rotation du cercle â”€â”€â”€
let circleAngle = 0;
let circleCenterX = 0;
let circleCenterY = 0;
let circleRadius = 0;
...
// â”€â”€â”€ Positionnement en cercle des projets + rotation lente â”€â”€â”€
function shuffleProjectIcons() {
  ...
  // Utiliser les dimensions rÃ©elles du desktop pour un centrage parfait
  ...
  // CrÃ©er les angles de base (un cercle complet)
  ...
  // Stocker l'angle de chaque icÃ´ne
  projectIcons.forEach((icon, i) => {
    icon.dataset.angle = angles[i];
  });

  // Sauvegarder les paramÃ¨tres du cercle pour la rotation
  circleCenterX = cx;
  circleCenterY = cy;
  circleRadius = radius;

  // Position initiale + dÃ©marrer la rotation
  circleAngle = 0;
  updateCirclePositions();
  startCircleRotation();
}
...
function updateCirclePositions() {
  ...
  icons.forEach(icon => {
    ...
    // Fondu doux : l'icÃ´ne derriÃ¨re s'estompe quand une autre passe devant
...
function startCircleRotation() {
  ...
  document.addEventListener('touchstart', e => {
    // Ignorer si on touche une icÃ´ne, une fenÃªtre ou le menu
  ...
  function tick(now) {
    ...
    // Decay naturelle de la vÃ©locitÃ©
...
// Pause la rotation pendant le drag d'une icÃ´ne (sauf si dÃ©jÃ  dÃ©tachÃ©e)
...
// Attendre que le layout soit stable avant de positionner les icÃ´nes (Ã©vite le dÃ©calage au lancement)
...
// â”€â”€â”€ Recalculer le cercle au resize â”€â”€â”€
...
// â”€â”€â”€ Ouvrir un projet Ã  l'honneur + scroller vers le bureau â”€â”€â”€
...
// â”€â”€â”€ Scroll-driven marquee â”€â”€â”€
...
if (marqueeTrack) {
  ...
  // Sur mobile : le swipe accÃ©lÃ¨re aussi le marquee
  let marqueeTouchY = 0;
...
// â”€â”€â”€ Anti-gravity hover : les voisins s'Ã©cartent au survol et reviennent â”€â”€â”€
(function() {
  if (isMobile) return;
  const icons = document.querySelectorAll('.desktop-icon');
...
function scrollToAbout() {
  ...
  // Sur mobile, libÃ©rer aussi le desktop du fullscreen lock
  if (isMobile) {
